...
const handleStepEnter = async (response) => {
    ...
    if (stepId.startsWith('2') || stepId.startsWith('4') || 
        ...
        try {
            ...
            switch(stepId) {
                case '2a':
                    ...
                    // 全画面表示モードに切り替え
                    if (figure) {
                        figure.classList.add('large-mode');
                        // チャートマネージャーを再初期化してサイズを更新
                ...
                case '2c':
                    // 通常表示モードに戻す
                    if (figure) figure.classList.remove('large-mode');
        ...
        // large-modeクラスを削除
        if (figure) {
            figure.classList.remove('large-mode');
        }
